...
(function($){
	...
	function Construct(el, options) 
	{
		this.opts = $.extend({	
			...
			cssPath: '/s/admin/css/editor/imperavi/',
			css: 'blank.css',
	...
	Construct.prototype = {
		...
		init: function()
		{
			...
			{
				files.push(this.opts.path + 'toolbars/' + this.opts.toolbar + '.js?v=2');
			}
		...
		{		
			...
			frameHtml += '<html><head><link media="all" type="text/css" href="' + this.opts.cssPath + this.opts.css + '" rel="stylesheet"></head>';
			frameHtml += '<body>';
			frameHtml += html;
			frameHtml += '</body></html>';
			return frameHtml;			
		...
		designMode: function()
		{
			if (this.doc)
			{
                ...
                if (this.$frame.data('loaded')) {
                    onIframeLoad.call();
                } else {
                    this.$frame.bind('load', onIframeLoad);
                }
			}
		},
		...
		docObserve: function()
		{
            $(this.doc.body).delegate("img", "click", function(e) { this.imageEdit(e); }.bind2(this));
        ...
        */
        imageEdit: function(e)
        {
            var handler = function()
            {
                ...
                $('#redactor_form_width').val($el.attr('width'));
                $('#redactor_form_height').val($el.attr('height'));

                $('#redactor_file_alt').val($el.attr('alt'));
                $('#redactor_form_image_fancy').attr('checked', $el.parent()[0].tagName == "A");

                var f = $el.css('float');
                if (f == 'none') {
                	f = 0;
                }
                $('#redactor_form_image_align').val(f);

            }.bind2(this);

            redactorActive = this;
            this.modalInit(RLANG.image, this.opts.path + 'plugins/image_edit.html?v=2', 450, 310, handler);
        },
        imageSave: function($el) {
        	var id = $el.attr('data-id'),
        		w = $('#redactor_form_width').val(),
        		h = $('#redactor_form_height').val(),
                float = $('#redactor_form_image_align').val(),
                fancy = $('#redactor_form_image_fancy').attr('checked') == 'checked'
        	if (id !== 0) {
	        	$.ajax({
	    	        type: "POST",
	    	        url: this.opts.image_settings + '/'+  id + '?t=' + new Date().getTime(),
	    	        data: {w: w, h: h},
	    	        success: function(data){
	    	        	var res = JSON.parse(data);
	    	        	$el.attr('src', res.path.replace(/\&amp\;/g,  "&"));
	    	        	$el.attr('width', w);
	    	        	$el.attr('height', h);
                        if ($el.parent('a')[0] == undefined) {
                            if (fancy) {
                                var full_path = $el.attr('src').match(/\&image\=(.*)$/);
                                if (full_path[1] != undefined) {
                                    $el.wrap('<a href="' + full_path[1] + '" rel="full_image"></a>');
                                }
                            } else {
                                //no fancy and need not
                            }
                        } else {
                            if (fancy) {
                                //has fancy and need
        ...
        deleteImage: function(el) {
        	...
        	if (id !== 0) {
        	    $.ajax({
        	        type: "GET",
        	        url: this.opts.image_delete + '/'+ id + '?t=' + new Date().getTime()
        	    });
        	}
        	$e.remove();
            this.modalClose();
        ...
        showImage: function()
        {
            ...
            redactorActive = this;
            this.modalInit(RLANG.image, this.opts.path + 'plugins/image.html?v=3', 450, 340, handler);
        },
        imageUploadCallback: function(data) {
            try {
                var res = JSON.parse(data);
                var full_path = res.full_path.replace(/\&amp\;/g, "&");
                var url = res.path.replace(/\&amp\;/g,  "&");
                var id = res.id;
                var w = res.w;
                var h = res.h;
                var alt = $('#redactor_file_alt').val();
                var className = '';
                var f = $('#redactor_form_image_align').val();
                if (f == 'left') {
                    className = 'class=="img_left"';
                } else if (f == 'right') {
                    className = 'class="img_right"';
                }
                var html = '<img width="' + w + '" height="' + h + '" data-id="' + id + '" alt="' + alt + '" src="' + url + '" ' + className + ' />';
                if ($('#redactor_form_image_fancy').attr('checked') == 'checked') {
                    html = '<a href="' + full_path + '" rel="full_image">' + html + '</a>'
                }
                redactorActive.$frame.get(0).contentWindow.focus();
                if ($.browser.msie) {
                ...
                //TODO image not selected. Server send 500
            }
        },
			...
			var handler = function()
			{
                ...
                if ($.browser.msie)
				{
					if (this.getParentNodeName() == 'A')
					{
                        ...
                        target_blank = this.insert_link_node.attr('target') == '_blank';
					}
					else
					{
						text = sel.text;
				...
				else
				{
					if (sel.anchorNode.parentNode.tagName == 'A')
					{
                        ...
                        target_blank = (parentNode.target == '_blank');
                ...
                $('#redactor_link_target').attr('checked', target_blank);
						
			}.bind2(this);

			this.modalInit(RLANG.link, this.opts.path + 'plugins/link.html?v=2', 400, 270, handler);
	
		},	
		insertLink: function()
		{
			var value = $('#redactor_link_text').val();
            var target_blank = $('#redactor_link_target').attr('checked') == 'checked';
			...
			var a = [
                '<a ',
                    'href="', mailto, $('#redactor_link_url').val(), '"',
		...
		showVideo: function()
		{
			...
			redactorActive = this;
			this.modalInit(RLANG.video, this.opts.path + 'plugins/video.html?v=3', 600, 360, function()
			{
